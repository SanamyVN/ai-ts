...
- `import { ZodType } from 'zod'` in `conversation.business.ts` — type comes from parameter

### getOrReconstructState()

No longer reads `session.outputSchema`. Builds only `{ threadId, resourceId }`.
...
| `session.mapper.ts` | Remove outputSchema mapping |
...
| `session-remote.mediator.ts` | Remove any outputSchema references |
