 ...
 *
 * `tenantId` and `purpose` are denormalized from `ai_sessions` at insert time
 * so billing scans stay single-table. Both fields are immutable on `ai_sessions`
 * after creation, bounding drift risk.
 *
...
export const aiSessionMessages = pgTable('ai_session_messages', {
  ...
  /** Denormalized from ai_sessions.tenant_id at insert time. */
